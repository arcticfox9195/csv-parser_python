...
def cut(inputList):
    ...
    for i in range(0, len(inputList), 6):
        for j in range(0, len(inputList[i]), 6):
            small_table = [inputList[i][j:j+6] for row in inputList[i:i+6]]
    ...
    for table in small_tables:
        ...
        for row in table:
            row_count += 1
            if len(row) < 6:
                for _ in range(len(row), 6): row.append('')

        if row_count < 6: 
            for _ in range(row_count, 6): table.append(['', '', '', '', '', ''])
