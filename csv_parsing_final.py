...
def cut(inputList):
    ...
    for i in range(0, len(inputList), 4):
        for j in range(0, len(inputList[i]), 4):
            small_table = [inputList[i][j:j+4] for row in inputList[i:i+4]]
            full = True
            ...
            cur = small_type_matrix[0]
            for k in range (1,4):
                if cur != small_type_matrix[k]:
                    row, col, choice =  select_row_and_col(small_type_matrix)
                    break
            repair_table, overflow = repair(small_table, row, col, choice)
            for k in range(4):
                inputList[i+k][j:j+4] = repair_table[k][0:4]
            inputList[row].insert(j+4, overflow)
