...
class Environment:
    ...
    def get_reward_row(self, action_row, record):
        ...
        for row in record:
            index += 1
            if row[0] == action_row: 
                record_find.append(index)
                find = True

        if find == True:
            print('row', action_row)
            return 5, record_find
        
        return -2, -1
    
    def get_reward_col(self, action_col, record, record_find):
        if len(record_find) == 0: return -1
...
def train_dqn_agent():
    ...
    for episode in range(episodes):
        ...
        while finish == 0:
            ...
            reward_row, record_find = env.get_reward_row(action_row, record_array)
            reward_col, record_find = env.get_reward_col(action_col, record_array, record_find)
            reward_choice = env.get_reward_choice(action_choice, record_array, record_find)
